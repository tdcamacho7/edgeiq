...
export default async function handler(req, res) {
  ...
  if (action === 'dk_status') {
    const OUT_DK = ['out','o','ir','il','injured reserve','injured list',
      'day-to-day','doubtful','inactive','suspended','na','gtd'];

    function buildStatusMap(players) {
      const statusMap = {};
      for (const p of players) {
        ...
        if (!name) continue;
        if (isOut) {
          const entry = { status: rawStatus, source: 'DraftKings', id };
          statusMap[name] = entry;
          const last = name.split(' ').pop();
          if (last.length > 2) statusMap[last] = entry;
          if (id) statusMap[`id:${id}`] = entry;  // exact ID match for client
        }
      }
      return statusMap;
    }

    // ── PATH A: draftGroupId provided → fetch all players in slate ──
    ...
    if (draftGrpId && draftGrpId !== '0') {
      ...
      for (const url of endpoints) {
        try {
          ...
          const statusMap = buildStatusMap(players);
          return res.status(200).json({
            success: true, outPlayers: statusMap,
            count: Object.keys(statusMap).length,
            source: 'DraftKings draftGroup API',
